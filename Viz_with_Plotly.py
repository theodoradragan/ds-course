...
def generate_pie_chart(launch_site):
    if launch_site == 'All Sites':
        ...
        fig = px.pie(values=spacex_df[spacex_df['Launch Site'] == str(launch_site)]['class'].value_counts(normalize=True), 
                     ...
                     title='Success launches for sites {}'.format(launch_site))
    return(fig)
...
def generate_payload_chart(launch_site, payload_mass):
    if launch_site == 'All Sites':
        fig = px.scatter(spacex_df[spacex_df['Payload Mass (kg)'].between(payload_mass[0], payload_mass[1])], 
                ...
                title='Correlation payload vs success for all sites')
    else:
        ...
        fig = px.scatter(df[df['Payload Mass (kg)'].between(payload_mass[0], payload_mass[1])], 
                ...
                color="Booster Version Category",
                hover_data=['Launch Site'],
                title='Correlation payload vs success for site {}'.format(launch_site))
    return(fig)
